...
int run_case(std::span<const std::byte> input, std::uint64_t fallback_seed)
{
    ...
    for (std::size_t step = 0U; step < 256U; ++step)
    {
        ...
        else if (op == 4U)
        {
            ...
            auto flat = chain.coalesce(voris::mem::resource_ref{system}, model.size(), alignof(std::max_align_t),
        ...
        else if (op == 5U && model.size() >= sizeof(std::uint16_t))
        {
            ...
            const auto expected = static_cast<std::uint16_t>((static_cast<std::uint16_t>(model[offset]) << 8U) |
        ...
        else if (op == 6U && !model.empty())
        {
            ...
            auto flat = sliced->coalesce(voris::mem::resource_ref{system}, count, alignof(std::max_align_t),
            ...
            if (!std::equal(model.begin() + static_cast<std::ptrdiff_t>(offset),
                            model.begin() + static_cast<std::ptrdiff_t>(offset + count), flat->const_view().data))
            {
                return 11;
            }
        }
...
int run_many_borrowed_segments_regression()
{
    ...
    auto flat_slice = sliced->coalesce(voris::mem::resource_ref{system}, 129U, alignof(std::max_align_t),
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
    {
        ...
        for (int arg = 1; arg < argc; ++arg)
        {
            ...
            if (token.starts_with("seed="))
            {
                ...
                if (result != 0)
                {
                    std::cerr << "buffer_chain_fuzz_input failed,seed=" << seed << ",code=" << result << '\n';
                    return result;
                }
                continue;
            }
        ...
        if (result != 0)
        {
            std::cerr << "buffer_chain_fuzz_input failed,bytes=" << input.size() << ",code=" << result << '\n';
            return result;
        }
    ...
    for (std::uint64_t seed = 1U; seed <= 32U; ++seed)
    {
        ...
        if (result != 0)
        {
            std::cerr << "buffer_chain_fuzz_smoke failed,seed=" << seed << ",code=" << result << '\n';
            return result;
        }
    }
