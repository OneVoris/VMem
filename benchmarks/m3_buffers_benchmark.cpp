...
[[nodiscard]] std::uint64_t elapsed_us(clock_type::time_point start,
                                       clock_type::time_point finish) {
    return static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count());
}

void print_line(const char* name,
                std::size_t segments,
                std::size_t operations,
                std::uint64_t micros) {
    std::cout << name << ",segments=" << segments << ",ops=" << operations
              << ",micros=" << micros << '\n';
}
