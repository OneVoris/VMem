...
void print_result(const char *name, std::size_t operations, std::uint64_t micros, voris::mem::usage_snapshot usage)
{
    std::cout << name << ",ops=" << operations << ",micros=" << micros << ",active_bytes=" << usage.active_bytes
              << ",reserved_bytes=" << usage.reserved_bytes << ",active_allocations=" << usage.active_allocations
              << ",failed_allocations=" << usage.failed_allocations << '\n';
}

void print_remote_result(const char *name, std::size_t remote_attempts, std::size_t drained_count,
                         std::size_t producer_count, std::size_t queue_capacity, std::uint64_t micros,
                         voris::mem::usage_snapshot usage, voris::mem::slab_remote_snapshot remote)
{
    std::cout << name << ",mode=smoke"
              << ",remote_attempts=" << remote_attempts << ",drain_released=" << drained_count
              << ",producer_threads=" << producer_count << ",queue_capacity=" << queue_capacity << ",micros=" << micros
              << ",active_bytes=" << usage.active_bytes << ",reserved_bytes=" << usage.reserved_bytes
              << ",active_allocations=" << usage.active_allocations
              << ",failed_allocations=" << usage.failed_allocations << ",queued=" << remote.queued_count
              << ",drained_total=" << remote.drained_count << ",saturated=" << remote.saturated_count
...
int main()
{
    ...
    print_remote_result("slab_remote_free_smoke", blocks.size(), drained, producer_count, remote_queue_capacity,
                        elapsed_us(start, finish), slab.usage(), remote);
    ...
    for (std::size_t thread_index = 0; thread_index < thread_count; ++thread_index)
    {
        threads.emplace_back([&] {
            ...
            {
                auto block = synchronized.allocate(make_allocation_request(32, alignof(std::max_align_t)));
                if (block)
                {
    ...
    finish = clock_type::now();
    print_result("synchronized_contention_smoke", thread_count * iterations * 2, elapsed_us(start, finish),
                 synchronized.usage());
