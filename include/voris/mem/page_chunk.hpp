...
class basic_page_chunk_manager {
public:
    explicit basic_page_chunk_manager(PageSource& source,
                                      page_chunk_options options = {}) noexcept
        : source_(&source), options_(options) {}

    [[nodiscard]] std::expected<page_allocation, errc>
    allocate_span(std::size_t requested_size) noexcept {
        if (requested_size == 0) {
        ...
        const bool use_direct = requested_size > *chunk_size ||
                                requested_size >= options_.direct_threshold;
