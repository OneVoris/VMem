...
class unique_buffer
{
  public:
    ...
    constexpr unique_buffer(unique_buffer &&other) noexcept
        : resource_(other.resource_), block_(other.block_), size_(other.size_), has_resource_(other.has_resource_)
    {
        other.block_ = {};
        other.size_ = 0U;
  ...
  private:
    friend std::expected<unique_buffer, errc> make_unique_buffer(resource_ref, std::size_t, std::size_t,
                                                                 memory_tag) noexcept;

    unique_buffer(resource_ref resource, allocation block) noexcept
        : resource_(resource), block_(block), has_resource_(true)
    {
    }
...
[[nodiscard]] inline std::expected<unique_buffer, errc> make_unique_buffer(resource_ref resource, std::size_t capacity,
