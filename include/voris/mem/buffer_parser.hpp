...
[[nodiscard]] std::expected<UInt, errc> peek_uint_be(const buffer_chain &chain, std::size_t offset = 0U) noexcept
{
...
template <unsigned_integer UInt>
[[nodiscard]] std::expected<UInt, errc> peek_uint_le(const buffer_chain &chain, std::size_t offset = 0U) noexcept
{
    auto end_offset = checked_add(offset, sizeof(UInt));
    if (!end_offset || *end_offset > chain.size())
    {
...
[[nodiscard]] inline std::optional<std::size_t> find_delimiter(const buffer_chain &chain, std::byte delimiter,
...
[[nodiscard]] inline std::expected<std::size_t, errc> copy_prefix(const buffer_chain &chain, mutable_buffer destination,
