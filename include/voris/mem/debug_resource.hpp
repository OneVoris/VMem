...
[[nodiscard]] inline debug_leak_diff diff_leak_snapshots(const debug_leak_snapshot &before,
                                                         const debug_leak_snapshot &after)
{
    debug_leak_diff diff;
    for (const auto &record : after.records)
    {
        const auto found =
            std::find_if(before.records.begin(), before.records.end(), [&](const debug_leak_record &candidate) {
                return candidate.block.data == record.block.data && candidate.generation == record.generation;
            });
    ...
    {
        const auto found =
            std::find_if(after.records.begin(), after.records.end(), [&](const debug_leak_record &candidate) {
                return candidate.block.data == record.block.data && candidate.generation == record.generation;
            });
...
class debug_resource
{
  public:
    ...
    [[nodiscard]] std::expected<debug_allocation, errc> allocate_block(const allocation_request &request) noexcept
    {
        ...
        const bool wants_guard_pages =
            options_.guard_pages_for_large_allocations && request.size >= options_.guard_page_threshold;
        if (wants_guard_pages)
        {
            ...
            else if (guarded.error() == errc::size_overflow || guarded.error() == errc::invalid_alignment)
            {
                count_failed_allocation();
        ...
        {
            std::fill_n(static_cast<std::byte *>(payload_block.data), request.size, options_.allocate_poison);
        }
    ...
    [[nodiscard]] std::expected<void, errc> deallocate_block(debug_allocation descriptor) noexcept
    {
  ...
  private:
    ...
    [[nodiscard]] std::expected<layout_info, errc> make_layout(std::size_t size, std::size_t alignment) const noexcept
    {
    ...
    [[nodiscard]] std::expected<raw_storage, errc> allocate_guarded(layout_info layout) noexcept
    {
    ...
    [[nodiscard]] std::expected<void *, errc> payload_from(void *storage, layout_info layout,
    ...
    [[nodiscard]] std::expected<std::size_t, errc> register_allocation(raw_storage storage, allocation block,
                                                                       const allocation_request &request) noexcept
    {
        ...
        else
        {
            try
            {
                records_.emplace(block.data, record{
                                                 .block = block,
                                                 .raw_block = storage.raw_block,
                                                 .reserved_span = storage.reserved_span,
                                                 .generation = generation,
    ...
    [[nodiscard]] std::expected<debug_allocation, errc> descriptor_for_current_locked(allocation block) const noexcept
    {
        ...
        if (!entry.allocated || entry.block.size != block.size || entry.block.alignment != block.alignment)
        {
            return std::unexpected(errc::wrong_owner);
        }
    ...
    [[nodiscard]] std::expected<void, errc> release_locked(debug_allocation descriptor, bool remote) noexcept
    {
        ...
        auto reserved_bytes =
            checked_sub(reserved_bytes_, entry.guard_pages ? entry.reserved_span.size : entry.raw_block.size);
        if (!reserved_bytes)
        {
            poison_redzones(entry.block.data, entry.block.size);
            return std::unexpected(reserved_bytes.error());
        ...
        if (options_.poison_on_free && entry.block.size != 0U)
        {
            ...
            std::fill_n(static_cast<std::byte *>(entry.block.data), entry.block.size, options_.free_poison);
        }
        ...
        if (!released)
        {
            ...
            {
                std::copy(saved_payload.begin(), saved_payload.end(), static_cast<std::byte *>(entry.block.data));
            }
            poison_redzones(entry.block.data, entry.block.size);
    ...
    void fill_redzones(void *payload, std::size_t size) const noexcept
    {
        ...
        std::fill_n(bytes - options_.redzone_size, options_.redzone_size, options_.redzone_poison);
    ...
    void unpoison_redzones(const record &entry) const noexcept
    {
        ...
        detail::asan_unpoison_memory_region(const_cast<std::byte *>(payload - entry.redzone_size), entry.redzone_size);
        detail::asan_unpoison_memory_region(const_cast<std::byte *>(payload + entry.block.size), entry.redzone_size);
    }
    ...
    [[nodiscard]] bool redzones_intact(const record &entry) const noexcept
    {
        ...
        return std::all_of(left, left + entry.redzone_size,
                           [&](std::byte byte) { return byte == options_.redzone_poison; }) &&
               std::all_of(right, right + entry.redzone_size,
                           [&](std::byte byte) { return byte == options_.redzone_poison; });
    }
