...
class pmr_resource_adapter
{
  public:
    ...
    [[nodiscard]] std::expected<void, errc> deallocate(allocation block) noexcept
    {
        ...
        if (found == active_.end() || found->second.size != block.size || found->second.alignment != block.alignment)
        {
            return std::unexpected(errc::wrong_owner);
        }
  ...
  private:
    [[nodiscard]] bool safe_upstream_deallocate(void *pointer, std::size_t size, std::size_t alignment) noexcept
    {
        try
        {
