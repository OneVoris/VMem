...
class shared_buffer
{
  public:
    ...
    constexpr shared_buffer(shared_buffer &&other) noexcept
        : control_(other.control_), handle_generation_(other.handle_generation_)
    {
    ...
    [[nodiscard]] std::expected<shared_buffer, errc> clone() const noexcept
    {
        ...
        for (;;)
        {
            ...
            if (control_->refcount.compare_exchange_weak(current, *next, std::memory_order_acq_rel,
    ...
    [[nodiscard]] std::expected<void, errc> reset() noexcept
    {
        ...
        for (;;)
        {
            ...
            if (control->refcount.compare_exchange_weak(current, *next, std::memory_order_acq_rel,
                                                        std::memory_order_acquire))
            {
                control_ = nullptr;
  ...
  private:
    friend std::expected<shared_buffer, errc> make_shared_buffer(resource_ref, std::size_t, std::size_t, memory_tag,
                                                                 shared_buffer_options) noexcept;

    constexpr shared_buffer(detail::shared_buffer_control *control, std::size_t handle_generation) noexcept
        : control_(control), handle_generation_(handle_generation)
    {
    }
...
[[nodiscard]] inline std::expected<shared_buffer, errc> make_shared_buffer(resource_ref resource, std::size_t capacity,
                                                                           ...
                                                                           shared_buffer_options options = {}) noexcept
{
    ...
    if (options.allow_cross_thread_final_release && traits.thread_safety != resource_thread_safety::thread_safe &&
    ...
    control->final_release_uses_remote =
        traits.thread_safety != resource_thread_safety::thread_safe && traits.supports_remote_deallocate;
