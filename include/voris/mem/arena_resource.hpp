...
class arena_resource
{
  ...
  private:
    ...
    [[nodiscard]] std::expected<void, errc> grow_for(std::size_t required_size, std::size_t alignment) noexcept
    {
        ...
        next_chunk_size_ = doubled ? std::min(*doubled, options_.max_chunk_size) : options_.max_chunk_size;
        return {};
    }

    [[nodiscard]] std::expected<allocation, errc> allocate_from_existing(std::size_t size,
    ...
    [[nodiscard]] bool contains_block(allocation block) const noexcept
    {
        ...
        for (const auto &current : chunks_)
        {
            ...
            if (chunk_end && begin >= chunk_begin && *end <= *chunk_end && begin % block.alignment == 0)
            {
                return true;
            }
        }
    ...
    [[nodiscard]] std::expected<void, errc> check_success_accounting(std::size_t size) const noexcept
    {
