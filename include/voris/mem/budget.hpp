...
public:
    ...
    explicit budget_event_sink(Sink& sink) noexcept
        : state_(std::addressof(sink)),
          emit_([](void* state, const budget_event& event) noexcept {
              (*static_cast<Sink*>(state))(event);
          }) {}
...
class budget_node {
public:
    explicit budget_node(budget_options options = {}) noexcept
    ...
    [[nodiscard]] std::expected<reservation_token, errc>
    reserve(std::size_t bytes, memory_tag tag = default_memory_tag) {
    ...
    release(std::size_t bytes, memory_tag tag = default_memory_tag) {
        try {
            const std::string tag_name{tag.name};
    ...
    template <typename Sink>
    void export_snapshots(Sink&& sink) const {
...
private:
    ...
    [[nodiscard]] std::expected<void, errc>
    can_reserve_locked(std::size_t bytes, const std::string& tag_name) const noexcept {
    ...
    [[nodiscard]] static std::expected<void, errc>
    prepare_reserve_locked(const std::vector<budget_node*>& nodes,
                           std::size_t bytes,
    ...
    [[nodiscard]] std::expected<void, errc>
    commit(const std::string& tag_name, std::size_t bytes) {
        if (bytes == 0U) {
            return {};
        }
    ...
    [[nodiscard]] std::expected<void, errc>
    rollback(const std::string& tag_name, std::size_t bytes) {
        if (bytes == 0U) {
            return {};
        }
    ...
    [[nodiscard]] std::expected<void, errc>
    release(const std::string& tag_name, std::size_t bytes) {
        if (bytes == 0U) {
            return {};
        }
    ...
    [[nodiscard]] std::expected<void, errc>
    release_reserved(const std::string& tag_name, std::size_t bytes) {
        try {
            std::vector<pending_event> events;
            {
                auto locked = lock_hierarchy();
    ...
    [[nodiscard]] std::expected<void, errc>
    can_commit_locked(const std::string& tag_name, std::size_t bytes) const noexcept {
    ...
    [[nodiscard]] std::expected<void, errc>
    can_release_reserved_locked(const std::string& tag_name, std::size_t bytes) const noexcept {
        if (reserved_bytes_ < bytes) {
            return std::unexpected(errc::wrong_owner);
        }
        const auto found = tag_accounts_.find(tag_name);
    ...
    [[nodiscard]] std::expected<void, errc>
    can_release_active_locked(const std::string& tag_name, std::size_t bytes) const noexcept {
    ...
    void release_reserved_locked(const std::string& tag_name,
    ...
    void release_active_locked(const std::string& tag_name,
                               std::size_t bytes) noexcept {
        const auto before_total = checked_add(reserved_bytes_, active_bytes_).value();
    ...
    [[nodiscard]] std::expected<void, errc>
    release_reserved_without_events(const std::string& tag_name, std::size_t bytes) {
    ...
    void prepare_upward_events_locked(std::string_view tag_name,
                                      std::size_t bytes,
        ...
        if (crossed_above(before_total, after_total, options_.limits.soft_limit)) {
            ...
            append_prepared_event_locked(budget_event_kind::soft_limit_exceeded,
                                         tag_name,
                                         after_reserved,
                                         active_bytes_,
                                         event_counters,
        ...
        if (crossed_above(before_total, after_total, options_.limits.high_watermark)) {
            ...
            append_prepared_event_locked(budget_event_kind::high_watermark_crossed,
                                         tag_name,
                                         after_reserved,
                                         active_bytes_,
                                         event_counters,
    ...
    void prepare_downward_events_locked(std::string_view tag_name,
                                        std::size_t bytes,
                                        bool reserved_release,
                                        std::vector<pending_event>& events) const {
        const auto before_total = checked_add(reserved_bytes_, active_bytes_).value();
        const auto after_reserved =
            reserved_release ? checked_sub(reserved_bytes_, bytes).value() : reserved_bytes_;
        const auto after_active =
            reserved_release ? active_bytes_ : checked_sub(active_bytes_, bytes).value();
        ...
        if (options_.limits.low_watermark != std::numeric_limits<std::size_t>::max() &&
            before_total > options_.limits.low_watermark &&
            after_total <= options_.limits.low_watermark) {
            budget_event_counters event_counters = counters_;
            ++event_counters.low_watermark_events;
            append_prepared_event_locked(budget_event_kind::low_watermark_crossed,
                                         tag_name,
                                         after_reserved,
                                         after_active,
                                         event_counters,
    ...
    void append_prepared_event_locked(budget_event_kind kind,
                                      std::string_view tag_name,
                                      std::size_t reserved_bytes,
                                      std::size_t active_bytes,
                                      budget_event_counters event_counters,
                                      std::vector<pending_event>& events) const {
    ...
    void apply_upward_counters_locked(std::size_t before_total,
                                      std::size_t after_total) noexcept {
        if (crossed_above(before_total, after_total, options_.limits.soft_limit)) {
    ...
    void apply_downward_counters_locked(std::size_t before_total,
                                        std::size_t after_total) noexcept {
        if (options_.limits.low_watermark != std::numeric_limits<std::size_t>::max() &&
            before_total > options_.limits.low_watermark &&
            after_total <= options_.limits.low_watermark) {
    ...
    crossed_above(std::size_t before, std::size_t after, std::size_t limit) noexcept {
    ...
    void erase_empty_tag_locked(const std::string& tag_name) {
        const auto found = tag_accounts_.find(tag_name);
        if (found != tag_accounts_.end() && found->second.reserved_bytes == 0U &&
            found->second.active_bytes == 0U) {
            tag_accounts_.erase(found);
        }
    }

    [[nodiscard]] budget_snapshot
    make_snapshot_locked(const std::string& tag_name, const tag_accounting& account) const {
