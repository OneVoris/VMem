...
public:
    explicit fault_injection_resource(resource_ref upstream,
                                      fault_injection_options options = {}) noexcept
...
private:
    ...
    [[nodiscard]] std::expected<bool, errc>
    should_fail(std::size_t call, const allocation_request& request) noexcept {
        ...
        for (;;) {
            ...
            if (!next_bytes) {
                requested_bytes_.store(std::numeric_limits<std::size_t>::max(),
                                       std::memory_order_relaxed);
                return std::unexpected(next_bytes.error());
            }
            if (requested_bytes_.compare_exchange_weak(prior_bytes,
                                                       *next_bytes,
                                                       std::memory_order_relaxed,
                                                       std::memory_order_relaxed)) {
                if (options_.fail_after_requested_bytes !=
