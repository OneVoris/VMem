...
std::expected<page_span, errc> reserve_regular_pages(std::size_t reserve_size) noexcept
{
    ...
    void *pointer = mmap(nullptr, reserve_size, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
#if defined(_WIN32)
    void *pointer = VirtualAlloc(nullptr, *reserve_size, MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES, PAGE_READWRITE);
    if (pointer == nullptr)
    {
        return std::unexpected(errc::out_of_memory);
    }
...
#elif defined(__linux__) && defined(MAP_HUGETLB)
    void *pointer =
        mmap(nullptr, *reserve_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
    if (pointer == MAP_FAILED)
    {
        return std::unexpected(errc::out_of_memory);
    }
...
std::expected<page_span, errc> os_page_source::reserve(std::size_t size, page_source_options options) noexcept
{
    if (size == 0)
    {
        return page_span{};
