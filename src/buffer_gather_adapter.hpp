...
[[nodiscard]] inline std::expected<std::vector<neutral_gather_entry>, errc> to_neutral_gather(const buffer_chain &chain,
...
#if defined(_WIN32)
[[nodiscard]] inline std::expected<std::vector<WSABUF>, errc> to_windows_wsabuf(const buffer_chain &chain,
                                                                                std::size_t max_segments)
{
    if (chain.segment_count() > max_segments)
    {
...
[[nodiscard]] inline std::expected<std::vector<iovec>, errc> to_posix_iovec(const buffer_chain &chain,
                                                                            std::size_t max_segments)
{
    if (chain.segment_count() > max_segments)
    {
